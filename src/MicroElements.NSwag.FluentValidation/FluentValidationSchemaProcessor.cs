...
namespace ZymLabs.NSwag.FluentValidation
{
    ...
    public class FluentValidationSchemaProcessor : ISchemaProcessor
    {
        ...
        public void Process(SchemaProcessorContext context)
        {
            ...
            try
            {
                validator = _validatorFactory.GetValidator(schemaType);
            }
            catch (Exception e)
            {
                _logger.LogWarning(0, e, $"GetValidator for type '{schemaType}' fails.");
            }
            ...
            _logger.LogDebug($"Applying FluentValidation rules to swagger schema for type '{schemaType}'.");
            ...
            {
                _logger.LogWarning(0, e, $"Applying IncludeRules for type '{schemaType}' fails.");
            }
                ...
                new FluentValidationRule("Length")
                {
                    ...
                    Apply = context =>
                    {
                        ...
                        if (lengthValidator.Max > 0)
                            schemaProperty.MaxLength = lengthValidator.Max;

                        if (lengthValidator is MinimumLengthValidator
                            || lengthValidator is ExactLengthValidator
                            || schemaProperty.MinLength == null)
                            schemaProperty.MinLength = lengthValidator.Min;
                    },
                },
